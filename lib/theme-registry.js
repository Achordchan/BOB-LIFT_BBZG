...
const PAGE_SETTING_KEY_PATTERN = /^[a-z][a-zA-Z0-9]*$/;
...
function createThemeRegistry(options) {
  ...
  function readTheme(directoryName) {
    ...
    const rawPageSettings = manifest.pageSettings && typeof manifest.pageSettings === 'object'
      ? manifest.pageSettings
      : {};
    const rawFields = Array.isArray(rawPageSettings.fields) ? rawPageSettings.fields : [];
    const settingKeys = new Set();
    const pageSettingFields = rawFields.map((field) => {
      const key = String((field && field.key) || '').trim();
      const label = String((field && field.label) || '').trim();
      if (!PAGE_SETTING_KEY_PATTERN.test(key) || !label || settingKeys.has(key)) {
        throw new Error(`主题 ${directoryName} 的首页设置字段无效`);
      }
      settingKeys.add(key);
      return {
        key,
        ...
        maxLength: Number.isInteger(field.maxLength) && field.maxLength > 0
          ? Math.min(field.maxLength, 200)
          : 60,
    ...
    return {
      ...
      pageSettings: {
        title: String(rawPageSettings.title || '首页文案'),
        description: String(rawPageSettings.description || ''),
        fields: pageSettingFields
      },
      entryPath
    };
  }
