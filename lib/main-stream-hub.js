...
function createMainStreamHub(deps) {
  const { getData, dataPath, getActiveTheme, getPageSettings } = deps || {};
  ...
  function buildPageSettings(data) {
    if (typeof getPageSettings === 'function') {
      return getPageSettings(data);
    }
    ...
    const settings = data && data.pageSettings;
    return settings && typeof settings === 'object' ? { ...defaults, ...settings } : defaults;
  }

  function buildTheme(data) {
    if (typeof getActiveTheme !== 'function') return null;
    const activeTheme = getActiveTheme(data);
    if (!activeTheme) return null;
    return {
      activeThemeId: activeTheme.id,
      updatedAt: data && data.themeSettings ? data.themeSettings.updatedAt : undefined
    };
  }
  ...
  function buildSnapshotFromData(data) {
    if (!data || typeof data !== 'object') {
      return {
        ...
        recentActivity: [],
        pageSettings: buildPageSettings(null),
    ...
    return {
      ...
      pageSettings: buildPageSettings(data),
      ...
      sseClients: getClientCount('screen')
    };
  }
  ...
  function closeClient(client, logDisconnect) {
  ...
  function registerRoutes(app) {
    ...
    app.get('/api/stream/main', (req, res) => {
      ...
      const client = {
        res,
        type: String(req.query && req.query.client || 'screen') === 'admin' ? 'admin' : 'screen',
        closed: false,
        heartbeatTimer: null
      };
  ...
  function getClientCount(type = 'screen') {
    let count = 0;
    clients.forEach(client => {
      if (!client.closed && client.type === type) count += 1;
    });
    return count;
  }
