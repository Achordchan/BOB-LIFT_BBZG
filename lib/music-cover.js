...
function normalizeMusicCover(value, localHostname = '') {
  ...
  try {
    ...
    if (relative || url.hostname === String(localHostname).toLowerCase()) {
      const staticImage = /^\/(?:images|uploads|covers|music|admin-app\/assets)\/.+\.(?:png|jpe?g|gif|webp|avif|bmp|svg|ico)$/.test(pathname);
      return staticImage || readOnly ? (relative ? raw : url.pathname + url.search) : '';
    }
...
function displayMusicCover(value, localHostname = '') {
  ...
  return /^https?:\/\//.test(normalized)
    ? `/api/public/music/cover-image?${new URLSearchParams({ url: normalized })}` : normalized;
}
