...
// 读缓存：按 mtime+size 判断文件是否变化，命中时跳过磁盘读。
...
function readRawCached(dataPath) {
  ...
  rawCache.set(dataPath, { mtimeMs: stat.mtimeMs, size: stat.size, raw });
  return raw;
}
...
function primeRawCache(dataPath, payload) {
  try {
    ...
    rawCache.set(dataPath, { mtimeMs: stat.mtimeMs, size: stat.size, raw: payload });
  } catch {
    rawCache.delete(dataPath);
  }
}
