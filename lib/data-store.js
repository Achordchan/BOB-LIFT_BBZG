...
// 读缓存：按 ino+mtime+ctime+size 判断文件是否变化，命中时跳过磁盘读。
// 本进程写入走 rename（inode 必变），外部原地改写由 ctime/mtime 兜底，
// 应对 NAS 等粗时间戳文件系统上同尺寸改写的误命中。
// 每次 getData 仍返回独立 JSON.parse 结果，保持调用方各自持有快照的语义。
...
function readRawCached(dataPath) {
  let stat = null;
  try {
    stat = fs.statSync(dataPath);
  } catch {
    rawCache.delete(dataPath);
    return fs.readFileSync(dataPath, 'utf8');
  }
  const cached = rawCache.get(dataPath);
  const key = statCacheKey(stat);
  if (cached && cached.key === key) {
    return cached.raw;
  }
  const raw = fs.readFileSync(dataPath, 'utf8');
  rawCache.set(dataPath, { key, raw });
  return raw;
}

function primeRawCache(dataPath, payload) {
  try {
    const stat = fs.statSync(dataPath);
    rawCache.set(dataPath, { key: statCacheKey(stat), raw: payload });
  } catch {
    rawCache.delete(dataPath);
  }
}
...
function getData(dataPath) {
  ...
  const raw = readRawCached(dataPath);
...
function writeAtomically(dataPath, data) {
  ...
  primeRawCache(dataPath, payload);
}
...
function saveDataCompat(dataPath, data) {
  if (syncBusy.get(dataPath)) {
    // 单线程下 busy 只可能是 mutator 内重入同步写；忙等会永久挂死进程，必须显式失败
    throw new Error(`检测到对 ${path.basename(dataPath)} 的重入同步写入，请在 mutator 外完成写操作`);
  }
...
function updateDataSync(dataPath, mutator) {
  if (syncBusy.get(dataPath)) {
    throw new Error(`检测到对 ${path.basename(dataPath)} 的重入同步更新，请在 mutator 外完成写操作`);
  }
  syncBusy.set(dataPath, true);
  try {
    if (shouldProfileDataIo) console.time('数据更新时间');
