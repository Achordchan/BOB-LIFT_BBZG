...
const AUDIT_RULES = [
  // 登录 / 账号安全（登录失败也记录）
  // 成败以本次响应为准：管理员登录成功跳 /admin，失败跳 /login?error=...
  // （不能只看会话里是否已有身份：已登录状态下再次输错密码，旧会话仍在会被误判成功）
  { m: 'POST', re: /^\/login\/?$/, action: '登录后台', detail: (r) => pick(r.body, ['username', 'account']), always: true, success: (_r, res) => {
      const location = String((res && typeof res.getHeader === 'function' && res.getHeader('Location')) || '');
      if (location) return !/^\/login\b/.test(location);
      return Number(res && res.statusCode) < 400;
    } },
  ...
  // 员工端登录失败返回 4xx，成功返回 2xx；同样不依赖会话中是否已有身份
  { m: 'POST', re: /^\/api\/egg\/login\/?$/, action: '员工端登录', always: true, success: (_r, res) => Number(res && res.statusCode) < 400 },
  ...
  // 注意：/api/music/import-netease 立即返回 jobId、下载与写库是异步的，
  // 若在此记录会把随后失败的导入记成功，故改为在任务真正 done/error 时
  // 由 routes/music.js 显式 recordAudit（见该文件内的导入完成/失败处）。
  ...
  { m: 'POST', re: /^\/api\/egg\/set-broadcast-from-bilibili\/?$/, action: '员工从哔哩哔哩设置播报', detail: (r) => pick(r.body, ['id', 'neteaseId', 'name']) },
  { m: 'POST', re: /^\/api\/bilibili\/cookie\/?$/, action: '设置哔哩哔哩授权' },
  { m: 'DELETE', re: /^\/api\/bilibili\/cookie\/?$/, action: '清除哔哩哔哩授权' },
