...
// 递归脱敏：字符串走 sanitizeLogValue，对象/数组浅递归（限制深度，避免超大对象拖慢）
function deepSanitize(value, depth) {
  ...
  if (typeof value !== 'object' || d <= 0) return value;
...
/** 未捕获异常 / 未处理的 Promise 拒绝入日志 */
function installProcessHandlers() {
  process.on('uncaughtException', (error) => {
    logger.error('未捕获异常 uncaughtException', { tag: 'process', message: error && error.message, stack: error && error.stack });
  });
  process.on('unhandledRejection', (reason) => {
    const err = reason instanceof Error ? reason : new Error(String(reason));
    logger.error('未处理的 Promise 拒绝 unhandledRejection', { tag: 'process', message: err.message, stack: err.stack });
  });
}
