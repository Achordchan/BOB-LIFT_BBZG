...
后台显示网易云歌曲图片与 B 站视频封面，导入后保存封面地址；旧网易云导入记录若仍有来源 ID，会按需补取歌曲图片。封面写入和返回记录时均校验：相对地址仅允许 images、uploads、covers、music、admin-app/assets 下的静态图片，以及两个只读封面接口；绝对地址也不能指向业务 API。历史自定义 HTTP(S) 静态图片仍可使用，脚本协议、凭据 URL 和业务写入口会被过滤。
