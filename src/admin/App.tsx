...
import { Layout, Menu, Button, Typography, Space, Input, Tag } from 'antd';
import {
  ApiOutlined,
  AudioOutlined,
...
const pages: Record<PageKey, { title: string; sub: string; component: JSX.Element }> = {
  ...
  celebration: { title: '庆祝语管理', sub: '维护成交播报模板和变量占位符', component: <CelebrationPage /> },
  platforms: { title: '平台目标', sub: '维护平台销售目标、当前进度和首页展示方式', component: <PlatformPage /> },
  settings: { title: '首页设置', sub: '只管理首页文案配置，不改首页页面代码', component: <HomeSettingsPage /> },
  apis: { title: 'API 调试', sub: '集中测试成交、询盘、TTS 和系统诊断接口', component: <ApiDebugPage /> },
  system: { title: '系统设置', sub: '管理员密码、登录状态和后台运行说明', component: <SystemPage /> }
};
  ...
  return (
    <Layout className="admin-shell">
      ...
      <Layout>
        ...
        <Content className="admin-content">{current.component}</Content>
      </Layout>
    </Layout>
  );
}
